...
class Curiosity(metaclass=ABCMeta):
    """
    Base class for curiosity implementations, Curiosity is the idea of calculating additonal intrinsic reward for
    ...
    def reward(self, rewards: np.ndarray, states: np.ndarray, actions: np.ndarray) -> np.ndarray:
        """
        Returns the modified rewards with the instrinsic reward incorporated
    ...
    def to(self, device: torch.device, dtype: torch.dtype) -> None:
        ...
        :param dtype: ``torch.dtype`` to cast all the parameter to
        """        
        self.device = device
        self.dtype = dtype
