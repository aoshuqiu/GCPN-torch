...
class MlpICMModel(ICMModel):
    ...
    def forward(self, state: Tensor, next_state: Tensor, action: Tensor):
        ...
        # actions between actual state pairs calculated by InverseModel
...
class ICM(Curiosity):
    ...
    The overview of the idea is to reward the agent for exploring unseen states. Itis achieved by implementing two
    models. One called forward model that given the encoded state and encoded next_state predicts action that
    must have been taken to move from one state to the other. the final intrisic reward is the difference between
    ...
    def __init__(self, state_converter: Converter, action_converter: Converter, model_factory: ICMModelFactory,
        ...
        :param reward_scale: scales the intrinsic reward returned by the module. Can be used to control how big the
               intrinsic reward is
        ...
        :param intrinsic_reward_integration: balances the importance between extrinsic and intrinsic reward. Used when
               incorporating intrisic into extrinsic in the ``reward`` method
