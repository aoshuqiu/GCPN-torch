...
class Model(nn.Module, metaclass=ABCMeta):
    ...
    For ``LSTM`` implementations the model should also keep track of the internal state of the ``LSTM`` cells.

    ``factory`` method is here just to hlep keep static typing clean when construction ``Agent`` objects and should
    return a object thant will construct desired implementation of the ``Model``
    """
    ...
    def __init__(self, state_space: Converter, action_space: Converter):
        ...
        :param action_space: tuple with actoin space size
        """
        super().__init__()
    ...
    @abstractmethod
    def policy_logits(self, state: Tensor) -> Tensor:
        """
