...
class MLP(Model):
    """
    MLP model that is able to handle one dimensional state and action spaces. It does not care about time series.
    So each example returned by the ``Dataset`` returned from ``dataset`` method is a tuple of a single 
    eg. state, action and reward
