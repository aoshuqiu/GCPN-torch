...
class SubProcessEnv(Process):
    ...
    *Note*: If the environment steps into terminal state (``done == True``) it immediately reset the envrionment
...
class MultiEnv(Env):
    """
    Runs multiple environments simultaneously. Most of the methods of this envrionment returns ``np.ndarray`` with
    aggregated responses from environment instead of single values.

    *Note*: Since this environment runs multiple ones underneath it never finishes. If one of the envrionments returns
    ...
    def __init__(self, env_id, n_envs: int, reporter: Reporter = NoReporter()):
        ...
        self.metadata["render.modes"].extend(["rgb_array", "human"])
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[Dict]]:
        ...
        :param action: ``array`` of actions for each agent of shape N*(action space shape) where N is number of
               envrionments. First dimension length must be equal to number of environments
        :return: tuple of:
