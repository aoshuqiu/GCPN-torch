...
def discount(rewards: np.ndarray, estimate_of_last: np.ndarray, dones: np.ndarray, discount: float):
    ...
    If the ``reward`` array contains unfinished episode this function will use values from ``estimate_of_last`` to
    calculate the :math:`\gamma^n V_t{t+n-1}(S_{t+n})` term 
    ...
    *Note:* This function does not support n-step discounts calculation. For this functionality
    ...
    :param dones: _description_
    :param discount: _description_
    """    
